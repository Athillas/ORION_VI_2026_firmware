...
{
    if(doc["S_W_L"].is<JsonVariant>())
    {
        hcs.S_W_L     = doc["S_W_L"];
    }

    if(doc["S_W_R"].is<JsonVariant>())
    {
        hcs.S_W_R     = doc["S_W_R"];
    }

    if(doc["S_INNE"].is<JsonVariant>())
    {
        hcs.S_INNE    = doc["S_INNE"];
    }

    if(doc["S_SCI"].is<JsonVariant>())
    {
        hcs.S_SCI     = doc["S_SCI"];
    }

    if(doc["S_ELEC"].is<JsonVariant>())
    {
        hcs.S_ELEK    = doc["S_ELEC"];
    }

    if(doc["S_RAM"].is<JsonVariant>())
    {
        hcs.S_RAM     = doc["S_RAM"];
    }

    if(doc["FAN1_ON"].is<JsonVariant>())
    {
        hcs.FAN1_ON   = doc["FAN1_ON"];
    }

    if(doc["NEOPIXEL"].is<JsonVariant>())
    {
